...
if [ $# -eq 0 ]; then
    ...
    echo "Використання: $(basename "$0") файл1.odt [файл2.doc ...]"
    exit 1
fi
...
function Table(tbl)
  ...
  -- Функция добавления одной строки таблицы
  ...
  -- Сначала обрабатываем строки из HEAD.
  -- В некоторых DOCX именно здесь находятся ВСЕ данные,
  -- несмотря на то, что визуально это обычная таблица.
...
if [ ${#CONVERTED_FILES[@]} -gt 1 ]; then
    echo "----------------------------------------"
    read -r -p "Об'єднати конвертовані файли? (y/n): " ANSWER
